...
* **spec → plan → execute workflow** — feature development is now a three-command pipeline that replaces the single-shot PRP flow (ADR-020):
  * **`/spec-dc`** — writes a **behavior specification** (the WHAT) to `.context/specs/spec-<unix-ts>-<slug>.md`. Keeps dotcontext's **clarity assessment** and reference-material handling; describes observable behavior only, and **does not review ADRs**.
